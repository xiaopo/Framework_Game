...
namespace XLua.CSObjectWrap
{
    ...
    public class MapLoadMapModeWrap
    {
		public static void __Register(RealStatePtr L)
        {
		    ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
		    Utils.BeginObjectRegister(typeof(Map.LoadMapMode), L, translator, 0, 0, 0, 0);
			Utils.EndObjectRegister(typeof(Map.LoadMapMode), L, translator, null, null, null, null, null);
			
			Utils.BeginClassRegister(typeof(Map.LoadMapMode), L, null, 4, 0, 0);

            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Single", Map.LoadMapMode.Single);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Additive_Add", Map.LoadMapMode.Additive_Add);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Additive_Over", Map.LoadMapMode.Additive_Over);
            

			Utils.RegisterFunc(L, Utils.CLS_IDX, "__CastFrom", __CastFrom);
            
            Utils.EndClassRegister(typeof(Map.LoadMapMode), L, translator);
        }
        ...
        static int __CastFrom(RealStatePtr L)
		{
            ...
            if (lua_type == LuaTypes.LUA_TNUMBER)
            {
                translator.PushMapLoadMapMode(L, (Map.LoadMapMode)LuaAPI.xlua_tointeger(L, 1));
            }
            ...
            else if(lua_type == LuaTypes.LUA_TSTRING)
            {
                ...
                {
                    translator.PushMapLoadMapMode(L, Map.LoadMapMode.Single);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "Additive_Add"))
                {
                    translator.PushMapLoadMapMode(L, Map.LoadMapMode.Additive_Add);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "Additive_Over"))
                {
                    translator.PushMapLoadMapMode(L, Map.LoadMapMode.Additive_Over);
                }
				else
                {
                    return LuaAPI.luaL_error(L, "invalid string for Map.LoadMapMode!");
                }
            ...
            {
                return LuaAPI.luaL_error(L, "invalid lua type for Map.LoadMapMode! Expect number or string, got + " + lua_type);
            }
