...
namespace XLua.CSObjectWrap
{
    ...
    public class GameMSceneLoadMapModeWrap
    {
		public static void __Register(RealStatePtr L)
        {
		    ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
		    Utils.BeginObjectRegister(typeof(Game.MScene.LoadMapMode), L, translator, 0, 0, 0, 0);
			Utils.EndObjectRegister(typeof(Game.MScene.LoadMapMode), L, translator, null, null, null, null, null);
			
			Utils.BeginClassRegister(typeof(Game.MScene.LoadMapMode), L, null, 4, 0, 0);

            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Single", Game.MScene.LoadMapMode.Single);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Additive_Add", Game.MScene.LoadMapMode.Additive_Add);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Additive_Over", Game.MScene.LoadMapMode.Additive_Over);
            

			Utils.RegisterFunc(L, Utils.CLS_IDX, "__CastFrom", __CastFrom);
            
            Utils.EndClassRegister(typeof(Game.MScene.LoadMapMode), L, translator);
        }
        ...
        static int __CastFrom(RealStatePtr L)
		{
            ...
            if (lua_type == LuaTypes.LUA_TNUMBER)
            {
                translator.PushGameMSceneLoadMapMode(L, (Game.MScene.LoadMapMode)LuaAPI.xlua_tointeger(L, 1));
            }
            ...
            else if(lua_type == LuaTypes.LUA_TSTRING)
            {
                ...
                {
                    translator.PushGameMSceneLoadMapMode(L, Game.MScene.LoadMapMode.Single);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "Additive_Add"))
                {
                    translator.PushGameMSceneLoadMapMode(L, Game.MScene.LoadMapMode.Additive_Add);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "Additive_Over"))
                {
                    translator.PushGameMSceneLoadMapMode(L, Game.MScene.LoadMapMode.Additive_Over);
                }
				else
                {
                    return LuaAPI.luaL_error(L, "invalid string for Game.MScene.LoadMapMode!");
                }
            ...
            {
                return LuaAPI.luaL_error(L, "invalid lua type for Game.MScene.LoadMapMode! Expect number or string, got + " + lua_type);
            }
