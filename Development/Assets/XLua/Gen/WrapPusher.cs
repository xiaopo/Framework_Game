...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineUIScrollRectMovementType
        {
            static IniterAdderUnityEngineUIScrollRectMovementType()
            {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<Game.MScene.LoadMapMode>(translator.PushGameMSceneLoadMapMode, translator.Get, translator.UpdateGameMSceneLoadMapMode);
			
			}
        }
        
        static IniterAdderUnityEngineUIScrollRectMovementType s_IniterAdderUnityEngineUIScrollRectMovementType_dumb_obj = new IniterAdderUnityEngineUIScrollRectMovementType();
        static IniterAdderUnityEngineUIScrollRectMovementType IniterAdderUnityEngineUIScrollRectMovementType_dumb_obj {get{return s_IniterAdderUnityEngineUIScrollRectMovementType_dumb_obj;}}
        ...
        int GameMSceneLoadMapMode_TypeID = -1;
		int GameMSceneLoadMapMode_EnumRef = -1;
        
        public void PushGameMSceneLoadMapMode(RealStatePtr L, Game.MScene.LoadMapMode val)
        {
            if (GameMSceneLoadMapMode_TypeID == -1)
            {
			    bool is_first;
                GameMSceneLoadMapMode_TypeID = getTypeId(L, typeof(Game.MScene.LoadMapMode), out is_first);
				
				if (GameMSceneLoadMapMode_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(Game.MScene.LoadMapMode));
				    GameMSceneLoadMapMode_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, GameMSceneLoadMapMode_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, GameMSceneLoadMapMode_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for Game.MScene.LoadMapMode ,value="+val);
            }
			
			LuaAPI.lua_getref(L, GameMSceneLoadMapMode_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out Game.MScene.LoadMapMode val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != GameMSceneLoadMapMode_TypeID)
				{
				    throw new Exception("invalid userdata for Game.MScene.LoadMapMode");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for Game.MScene.LoadMapMode");
                }
				val = (Game.MScene.LoadMapMode)e;
                
            }
            else
            {
                val = (Game.MScene.LoadMapMode)objectCasters.GetCaster(typeof(Game.MScene.LoadMapMode))(L, index, null);
            }
        }
		
        public void UpdateGameMSceneLoadMapMode(RealStatePtr L, int index, Game.MScene.LoadMapMode val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for Game.MScene.LoadMapMode ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			if (type == typeof(UnityEngine.UI.ScrollRect.MovementType[]))
			{
			...
			else if (type == typeof(Game.MScene.LoadMapMode[]))
			{
			    Game.MScene.LoadMapMode[] array = obj as Game.MScene.LoadMapMode[];
				translator.PushGameMSceneLoadMapMode(L, array[index]);
				return true;
			}
            return false;
		}
