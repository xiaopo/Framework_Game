...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineUIScrollRectMovementType
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<Map.LoadMapMode>(translator.PushMapLoadMapMode, translator.Get, translator.UpdateMapLoadMapMode);
        ...
        int MapLoadMapMode_TypeID = -1;
		int MapLoadMapMode_EnumRef = -1;
        
        public void PushMapLoadMapMode(RealStatePtr L, Map.LoadMapMode val)
        {
            if (MapLoadMapMode_TypeID == -1)
            {
			    bool is_first;
                MapLoadMapMode_TypeID = getTypeId(L, typeof(Map.LoadMapMode), out is_first);
				
				if (MapLoadMapMode_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(Map.LoadMapMode));
				    MapLoadMapMode_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, MapLoadMapMode_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, MapLoadMapMode_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for Map.LoadMapMode ,value="+val);
            }
			
			LuaAPI.lua_getref(L, MapLoadMapMode_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out Map.LoadMapMode val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != MapLoadMapMode_TypeID)
				{
				    throw new Exception("invalid userdata for Map.LoadMapMode");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for Map.LoadMapMode");
                }
				val = (Map.LoadMapMode)e;
                
            }
            else
            {
                val = (Map.LoadMapMode)objectCasters.GetCaster(typeof(Map.LoadMapMode))(L, index, null);
            }
        }
		
        public void UpdateMapLoadMapMode(RealStatePtr L, int index, Map.LoadMapMode val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for Map.LoadMapMode ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(Map.LoadMapMode[]))
			{
			    Map.LoadMapMode[] array = obj as Map.LoadMapMode[];
				translator.PushMapLoadMapMode(L, array[index]);
				return true;
			}
            return false;
		}
